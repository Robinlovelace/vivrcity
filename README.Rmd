...
The package provides functions to simplify IDs and aggregate both counts and metadata.

```{r aggregation}
# Aggregate counts by simplified ID (sensor)
aggregated_counts <- aggregate_counts(counts, simplify_fn = id_simplify)
nrow(aggregated_counts) / nrow(counts)

# Aggregate metadata (returns sf object with one row per sensor and unioned geometry)
# Use name_simplify to aggregate based on the name column (where sensor ID usually is)
metadata_aggregated <- aggregate_meta(metadata_sf, simplify_fn = name_simplify)

# Plot aggregated metadata
qtm(metadata_aggregated)
```
